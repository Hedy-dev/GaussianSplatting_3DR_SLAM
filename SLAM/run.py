...
from arguments import ModelParams, PipelineParams, GlobalParams, get_combined_args
from gaussian_renderer import GaussianModel
from SLAM.utils.helper import load_LoFTR, get_pose_estimation_input
...
def trans_t_xyz(tx, ty, tz):
    ...
    return T
class SLAM:
    def draw_camera_path(self, runtime_info, viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, compute_grad_cov2d=True):
        ...
        camera_pose = Camera_Pose(world_to_cameraB,FoVx=runtime_info.FoVx,FoVy=runtime_info.FoVy,
                                image_width=runtime_info.image_width,image_height=runtime_info.image_height)
        camera_pose.cuda()
    ...
    def camera_pose_estimation(self, gaussians:GaussianModel, background:torch.tensor, pipeline:PipelineParams, globalParams:GlobalParams, runtime_info, output_path):
        # start pose & gt pose
        # Гомогенная матрица преобразования из системы координат камеры в мировую систему координат
        gt_pose_c2w=runtime_info.gt_pose_c2w
        ...
        start_pose_w2c=runtime_info.start_pose_w2c.cuda()
        # Перемещение тензора query_image, содержащего информацию об изображении запроса
        # query_image = 
        query_image = runtime_info.query_image.cuda()

        
        camera_pose = Camera_Pose(start_pose_w2c,FoVx=runtime_info.FoVx,FoVy=runtime_info.FoVy,
                                image_width=runtime_info.image_width,image_height=runtime_info.image_height)
        ...
        matching_flag= not globalParams.deprecate_matching # Стоит ли объявлять соответствующий модуль устаревшим с самого начала True
        num_iter_matching = 0

        # start optimizing
        optimizer = optim.Adam(camera_pose.parameters(),lr = globalParams.camera_pose_lr)
        ...
        camera_pose_r = Camera_Pose(start_pose_w2c_r,FoVx=runtime_info.FoVx,FoVy=runtime_info.FoVy,
                                image_width=runtime_info.image_width,image_height=runtime_info.image_height)
        camera_pose_r.cuda()
        for k in range(globalParams.pose_estimation_iter):
            # Выполняется рендеринг сцены с текущей камерой
            rendering = render(camera_pose,
                            gaussians, 
                            pipeline, 
                            background,
                            compute_grad_cov2d = globalParams.compute_grad_cov2d)#["render"]
            ...
            if matching_flag:
                ...
                loss_matching = loss_loftr(query_image,
                                        rendering,
                                        self.LoFTR_model,
                                        globalParams.confidence_threshold_LoFTR,
                                        globalParams.min_matching_points)
                ...
                if loss_matching is None:
                    ...
                    loss = globalParams.lambda_LoFTR *loss_matching + (1-globalParams.lambda_LoFTR)*loss_comparing
            ...
            # если модуль сопоставления устарел, используется только функция потерь сравнения
                ...
                new_lrate = globalParams.camera_pose_lr * (0.6 ** ((k - num_iter_matching + 1) / 50))
            ...
            if (k + 1) % 20 == 0 or k == 0:
                ...
                current_camera_pose = self.draw_camera_path(runtime_info, matrix_pose_c2w_to_top_camera, gaussians, 
                            pipeline, 
                            background,
                            compute_grad_cov2d = globalParams.compute_grad_cov2d)
                # a
                ...
                if globalParams.OVERLAY is True:
                    with torch.no_grad():
    ...
    #
        ...
        camera_b_view = to8b(render(camera_pose,
                                ...
                                pipeline, 
                                background,
                                compute_grad_cov2d = globalParams.compute_grad_cov2d).clone().permute(1, 2, 0).cpu().detach().numpy())
            # return_image = to8b(camera_b_view.clone().permute(2, 1, 0).cpu().detach().numpy().astype(np.uint8))
    ...
    def start(self):
        ...
        args, model, pipeline, global_params = get_combined_args()
        ...
        self.LoFTR_model=load_LoFTR(global_params.LoFTR_ckpt_path,global_params.LoFTR_temp_bug_fix)
        ...
        runtime_info=get_pose_estimation_input(obs_view,ast.literal_eval(args.delta))
        
        # pose estimation
        self.camera_pose_estimation(gaussians,background,pipeline,global_params,runtime_info,args.output_path)
